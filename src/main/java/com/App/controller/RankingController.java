...
public class RankingController {
	...
	@RequestMapping(value = "/updateRanking", method = RequestMethod.GET, produces = "application/json")
	public ResponseEntity<Object> updateRanking() {
		...
		if (ranking != null) {
			if (ranking.isWasProccessed()) {
				return new ResponseEntity<>(new GenericReturnMessage(0, "Aguarde até virar o dia"), HttpStatus.BAD_REQUEST);
			} else {
		...
		Collection<UserStats> listStats = userStatsRepository.getUserStatsFilter();
		...
		return new ResponseEntity<>(new GenericReturnMessage(0, "Ranking Atualizado"), HttpStatus.OK);
	}
